# -*- coding: utf-8 -*-
"""Update TW History DB with latest yfinance data"""
import sys
...
stocks = cur.fetchall()
print(f"Stocks to update: {len(stocks)}")
...
for sym, name in stocks:
    try:
        ...
        # Download from next day after last_date
        if last_date:
        ...
        updated += 1
        print(f"  {sym} {name}: {len(df)} rows added (last: {date_str})")
