...
st.set_page_config(page_title="Tina Scanner v3.0 — 台美股技術評分", page_icon="📊", layout="wide")

st.title("📊 Tina Scanner v3.0 — 台美股技術評分系端")



tw_tab, us_tab = st.tabs(["📊 台股", "🇺🇸 美股"])
...
try:
    ...
    if not twii_hist.empty and not spy_hist.empty:
        ...
        m[0].metric("加權指數", f"{twii_close:,.0f}", f"{twii_chg:+.2f}%")

        m[1].metric("加權指數 RSI", f"{twii_rsi:.0f}" if twii_rsi else "N/A")

        m[2].metric("TW狀態", rsi_label_tw)

        m[3].metric("S&P500", f"{spy_close:,.0f}", f"{spy_chg:+.2f}%")

        m[4].metric("標普500 RSI", f"{spy_rsi:.0f}" if spy_rsi else "N/A")
...
with tw_tab:
    ...
    with col_side:
        ...
        tw_cat = st.selectbox("類別", list(TW_CATS.keys()), key="tw_cat")
        ...
        with st.form(key="grade_filter_tw"):
            ...
            if gall.form_submit_button("全", help="全選", use_container_width=True):
                pass  # Checkbox values are auto-managed by Streamlit via key=
            tw_grade = [g for g, on in zip(["A","B","C","D"], [g_a, g_b, g_c, g_d]) if on]

        tw_score_min = st.slider("最低評分", 0, 1000, 0, key="tw_score")
        tw_rsi_max = st.slider("RSI 上限", 30, 100, 100, key="tw_rsi")

        tw_macd_filter = st.checkbox("排除 MACD < 0", value=False, key="tw_macd_filter")

        codes = TW_CATS.get(tw_cat, [])

        st.info(f"{len(codes)} 檔股目")

        analyze_tw = st.button("🔍 開姛分析", type="primary", use_container_width=True, key="btn_tw_analyze")
    ...
    if analyze_tw:
        ...
        with st.spinner("分析中... (首批約需5秒)"):
            ...
            st.info(f"完成 {len(results)} 檔股目\uff0c耗時 {elapsed:.1f}秒")
    ...
    if results:
        ...
        st.success(f"共 {len(results)} 檔 | 篇選後 {len(filtered)} 檔")
    ...
    if filtered:
        ...
        with st.expander("📨 發送到 Telegram"):

            grade_filter = st.multiselect("評級篇選", ["A","B","C","D"], default=["A","B","C","D"], key="tw_grade_send")

            grade_filtered = [r for r in filtered if r['tier'] in grade_filter]

            sel = st.multiselect("选回", [f"[{r['tier']}] S={r['score']:.0f} {r['code']} ${r['price']:.0f}" for r in grade_filtered], key="tw_sel")
            ...
            if r1.button(f"發送 ({sc}) 等級 {','.join(grade_filter)}", disabled=(sc==0), use_container_width=True, key="tw_batch_send"):
                ...
                with st.spinner("發送中..."):
                    ...
                    if ok_all:
                        ...
                        st.success(f"已發送 {sc} 檔 ({len(chunks)} 則)")

            if r2.button(f"發送全部 ({len(grade_filtered)}) 等級 {','.join(grade_filter)}", use_container_width=True):
                ...
                with st.spinner("發送中..."):
                    ...
                    if ok_all:
                        ...
                        st.success(f"已發送 {len(grade_filtered)} 檔 ({len(chunks)} 則)")
        ...
        if results:
            ...
            st.download_button("📥 下載 CSV", csv, f"tw_{cat_saved}_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv", key="tw_csv")
    ...
    with spacer:
        ...
        st.caption('電自發送' if st.session_state.get('tw_auto_send') else '電自手動發送')
    ...
    with col_code:
        ...
        single_code = st.text_input("股目代碼", "2330", key="tw_single_code").strip().upper()

    with col_btn:

        st.write(" ")

        do_single = st.button("🔍 分析", type="primary", use_container_width=True, key="btn_tw_single")

    if do_single or st.session_state.get('tw_analyzed', False):

        if do_single:
            with st.spinner(f"分析中 {single_code}..."):
        ...
        if not r:
            ...
            with st.form(key="tw_single_tg_form", clear_on_submit=False):
                ...
                submitted = st.form_submit_button("📤 發送到 Telegram", use_container_width=True)
                # submitted debug
                if submitted:
                    st.info("表單已送出！")
                    st.info("發送中...")
...
with us_tab:
    ...
    with col_side:
        ...
        us_cat = st.selectbox("類別", list(US_CATS.keys()), key="us_cat")
        ...
        with st.form(key="grade_filter_us"):
            ...
            if uall.form_submit_button("全", help="全選", use_container_width=True):
                pass  # Checkbox values are auto-managed by Streamlit via key=
            us_grade = [g for g, on in zip(["A","B","C","D"], [u_a, u_b, u_c, u_d]) if on]

        us_score_min = st.slider("最低評分", 0, 1000, 0, key="us_score")
        us_rsi_max = st.slider("RSI 上限", 30, 100, 100, key="us_rsi")
        ...
        analyze_us = st.button("🔍 開姛分析", type="primary", use_container_width=True, key="btn_us_analyze")
    ...
    if filtered:
        ...
        with st.expander("📨 發送到 Telegram"):
            ...
            grade_filter = st.multiselect("評級篇選", ["A","B","C","D"], default=["A","B","C","D"], key="us_grade_send")

            grade_filtered = [r for r in filtered if r['tier'] in grade_filter]

            sel = st.multiselect("选回", [f"[{r['tier']}] S={r['score']:.0f} {r['code']} ${r['price']:.0f}" for r in grade_filtered], key="us_sel")
            ...
            if r1.button(f"發送 ({sc}) 等級 {','.join(grade_filter)}", disabled=(sc==0), use_container_width=True, key="us_batch_send"):
        ...
        if results:
            ...
            st.download_button("📥 下載 CSV", csv, f"us_{cat_saved}_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv", key="us_csv")
    ...
    with spacer:
        ...
        st.caption('電自發送' if st.session_state.get('us_auto_send') else '電自手動發送')
    ...
    with col_code:
        ...
        us_single_code = st.text_input("股目代碼", "NVDA", key="us_single_code").strip().upper()

    with col_btn:

        st.write(" ")

        do_us_single = st.button("🔍 分析", type="primary", use_container_width=True, key="btn_us_single")

    if do_us_single or st.session_state.get('us_analyzed', False):

        if do_us_single:
            with st.spinner("分析中 " + us_single_code + "..."):
