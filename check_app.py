...
ticker = yf.Ticker('APP')
info = ticker.info

print('=== AppLovin (APP) 基本資料 ===')
...
for date, row in hist.iterrows():
    print(str(date)[:10], '開', round(row['Open'],2), '高', round(row['High'],2), '低', round(row['Low'],2), '收', round(row['Close'],2), '量', f"{int(row['Volume']):,}")