...
US_CATS = {
    ...
    'AI ?箇?閮剜': ['NVDA','AMD','AVGO','MRVL','AMZN','MSFT','GOOGL','META','ANET','VRT','DELL','PLTR','NOW','ORCL','COHR','LITE','GLW','AMSC','NVT','SBGSY','EQIX','DLR','AMKR'],

    '??蝘?': ['PYPL','SQ','AFRM','COIN','HOOD','DB','AFC','GREM','UPST','LC','RBLX','NU','SOFI'],

    '?餃?頠?蝬': ['TSLA','RIVN','LCID','F','GM','HYLN','ENPH','SEDG','SPWR','FSLR','RUN','ALB','NIO','XPEV','CHPT','BLNK','CCID','NOVA'],
...
TW_CATS["?券"] = sorted(all_tw, key=lambda x: (0, int(x)) if x.isdigit() else (1, x))[:999]
...
US_CATS["?券"] = sorted(all_us, key=lambda x: x)[:999]



TW_NAMES = {

    "2330": "?啁???, "2454": "?舐蝘?, "2317": "暾餅絲", "2382": "撱??",

    "3034": "蝺舐?", "3665": "蝛斐", "2881": "撖??, "2603": "?瑟旨",

    "2303": "?舫", "1216": "蝯曹?", "0050": "?之?啁50", "0056": "?之擃??,

    "00646": "撖S&P500", "00662": "撖NASDAQ", "00713": "?之擃雿郭",

    "00757": "蝯曹?憭佝ANG+", "00927": "蝯曹???芯?", "00878": "?陸瘞貊?擃??,

    "00900": "撖ESG", "00902": "????", "00906": "?勗?芷擃??,

    "3217": "3217", "2401": "2401", "3527": "3527", "4749": "蝢?",
...
US_NAMES = {
    ...
    'VOO': "Vanguard S&P 500", 'IVV': "iShares Core S&P 500", 'SPY': "SPDR S&P 500", 'VTI': "Vanguard ?函?撣",

    # ??? ETF - 蝘????????

    'QQQ': "Invesco QQQ", 'VUG': "Vanguard Growth", 'SOXX': "iShares ??擃?, 'SMH': "VanEck ??擃?,

    # ??? ETF - ?⊥??????

    'SCHD': "Schwab 擃??, 'VYM': "Vanguard 擃??拍?", 'VTV': "Vanguard Value", 'DGRO': "iShares ?⊥憓",

    # ??? ETF - ?萄??蝵????

    'BND': "Vanguard 蝬??萄", 'TLT': "iShares ?瑕予????, 'SHY': "iShares ?剖予????, 'AGG': "iShares 蝬???,

    # ??? ETF - ?函??隞????

    'VT': "Vanguard ?函??∠巨", 'VXUS': "Vanguard ???∪?", 'VWO': "Vanguard ?啗?撣",
    ...
    'USD': "ProShares 2x ??擃?, 'ROM': "ProShares 2x 蝘?", 'UYG': "ProShares 2x ??", 'DIG': "ProShares 2x 瘝寞除", 'UXI': "ProShares 2x 撌交平",

    'UBT': "ProShares 2x ?瑕", 'UST': "ProShares 2x 銝剖", 'UUP': "Invesco 蝢??",
...
try:
    ...
    if not twii_hist.empty and not spy_hist.empty:
        ...
        rsi_label_tw = "[ERR] ?" if twii_rsi and twii_rsi > 70 else ("[Y] ??" if twii_rsi and twii_rsi > 50 else "[G] 銝剜?)

        rsi_label_us = "[ERR] ?" if spy_rsi and spy_rsi > 70 else ("[Y] ??" if spy_rsi and spy_rsi > 50 else "[G] 銝剜?)
...
with tw_tab:
    ...
    with col_side:
        ...
        if gall.button("??, help="Select All", use_container_width=True):
        ...
        tw_macd_filter = st.checkbox("? MACD < 0", value=False, key="tw_macd_filter")
    ...
    with auto_col:
        ...
        st.session_state['tw_auto_send'] = st.toggle('??敺???Telegram', value=st.session_state['tw_auto_send'], key='tw_auto_send_toggle')

    with spacer:

        st.caption('ON = ?芸??潮? if st.session_state.get('tw_auto_send') else 'OFF = ???潮?)
    ...
    if do_single:
        ...
        if r:
            ...
            if kd_ok and ma_bull and macd_v>0 and rsi_v<65:
                act,col,trend = "撘瑕?銋啣","green","憭"
            elif score>=700 and rsi_v<70:
                act,col,trend = "銋啣","blue","憭"
            elif score>=400:
                act,col,trend = "閫??,"gray","銝剜?
            elif rsi_v>80 or bb_v>90:
                act,col,trend = "?閫??,"orange","霅西死"
            else:
                act,col,trend = "皜?","red","蝛粹"
...
with us_tab:
    ...
    with col_side:
        ...
        if uall.button("??, help="Select All", use_container_width=True):
        ...
        us_macd_filter = st.checkbox("? MACD < 0", value=False, key="us_macd_filter")
    ...
    with auto_col:
        ...
        st.session_state['us_auto_send'] = st.toggle('??敺???Telegram', value=st.session_state['us_auto_send'], key='us_auto_send_toggle')

    with spacer:

        st.caption('ON = ?芸??潮? if st.session_state.get('us_auto_send') else 'OFF = ???潮?)
