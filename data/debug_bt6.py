...
# Generate signals - same logic as backtest_framework
params = {
...
in_pos = False; entry_p = 0.0; entry_atr = 0.0; entry_idx = 0
...
for i in range(50, len(df)):
    ...
    if not in_pos:
        ...
        if can_entry:
            ...
            in_pos = True; entry_p = price; entry_atr = atr14[i]; entry_idx = i
    else:
...
# Now test run_backtest's trade simulation logic
capital = 1_000_000; position = 0; shares = 0; entry_price = 0; entry_date = None; trades = []

print('Testing iterrows access:')
for idx_count, (i, row) in enumerate(df.iterrows()):
    ...
    if es == 1 and position == 0:
        ...
        entry_price = price; position = 1; entry_date = date
        print(f'  [{idx_count}] BUY at {date.date()} {price:.2f} x{shares} (es={es}, xs={xs})')
        ...
        print(f'  [{idx_count}] SELL at {date.date()} {price:.2f} pnl={pnl_pct:+.2f}% (es={es}, xs={xs})')
        position = 0; shares = 0
