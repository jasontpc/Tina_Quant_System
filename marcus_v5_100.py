...
TIER3_BLUE = [
    ...
    # ETF
    '0050', '0056', '00646', '00662', '00891', '00713', '00751', '00752',
    '00878', '00900', '00929', '00937', '00941', '00946', '00949',
]
...
TIER3 = [s for s in TIER3_BLUE if s not in BLACKLIST][:55]
...
def backtest(inst_map, days=180):
    ...
    for idx, code in enumerate(ALL_STOCKS):
        try:
            # ETF uses .TW, but some use different suffixes
            ticker_str = code + '.TW' if not code.startswith('00') else code + '.TW'
            h = yf.Ticker(ticker_str).history(start=start_date, end=end_date)
...
for tier in ['tier1', 'tier2', 'tier3', 'all']:
    r = results[tier]
    if r:
        name = {'tier1': 'Tier1 科技', 'tier2': 'Tier2 供應鏈', 'tier3': 'Tier3 藍籌/ETF', 'all': '整體'}[tier]
        print(f'\n{name}:')
...
output = {
    ...
    'stock_list': {'tier1': TIER1, 'tier2': TIER2, 'tier3': TIER3},
