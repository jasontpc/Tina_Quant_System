# -*- coding: utf-8 -*-
"""
Nana v5.25 - 精選策略回歸 (Cycle 37)
目標: WR>55%, Avg>3.0%, 交易量>300筆

核心修正 (相較 v5.24 回歸問題):
1. 回歸 v5.23 精選股票池：只保留核心4檔（2382, 2884, 2474, 3017）
2. 擴充黑名單至13檔：加入 4938 (WR=43.1%, Avg=-0.48%), 2353 (WR=41.4%, Avg=-1.02%)
3. 剔除弱勢 ETF：移除 00662 (Avg=-0.14%), 00646 (Avg=-0.13%)
4. 維持 ETF 池 6 檔：0050, 0056, 00891, 00713, 00900, 00902
5. 黑名單：2451,2330,1605,6230,2454,2308,3034,3413,2458,2379,8081,4938,2353
"""
import sys
...
BLACKLIST = {
    ...
    '4938',  # Avg=-0.48%, WR=43.1% ← v5.25 新增
    '2353',  # Avg=-1.02%, WR=41.4% ← v5.25 新增
}

# ==================== 精選股票池 (核心4檔) ====================
# 依 v5.24 個別表現挑選：2382(T1,WR=56.3%,Avg=0.26%), 3017(T1,WR=65.0%,Avg=3.51%),
# 2474(T2,WR=47.9%,Avg=-0.20%), 2884(金融,Avg待驗)
CORE_STOCKS = [
    '2382',  # 廣達  - v5.24: 167筆, WR=56.3%, Avg=0.26%
    '2884',  # 玉山金 - 金融白馬
    '2474',  # 可成  - v5.24: 119筆, WR=47.9%, Avg=-0.20%
    '3017',  # 奇鋐  - v5.24: 157筆, WR=65.0%, Avg=3.51%
]

# ==================== 精選 ETF 池 (6檔) ====================
# 剔除 00662(Avg=-0.14%), 00646(Avg=-0.13%)
CORE_ETF = [
    '0050',   # 元大台灣50    - v5.24: 126筆, WR=60.3%, Avg=1.06%
    '0056',   # 元大高股息   - v5.24: 144筆, WR=54.2%, Avg=0.30%
    '00891',  # 中信金特     - v5.24: 193筆, WR=58.5%, Avg=0.96%
    '00713',  # 統一台灣高息 - v5.24: 134筆, WR=50.0%, Avg=0.02%
    '00900',  # 永豐ESG      - v5.24: 187筆, WR=52.9%, Avg=0.19%
    '00902',  # 兆豐藍籌30   - v5.24: 192筆, WR=65.1%, Avg=1.28%
]

STOCK_NAMES = {
    '2382':'廣達','2884':'玉山金','2474':'可成','3017':'奇鋐',
    '0050':'元大台灣50','0056':'元大高股息','00891':'中信金特',
    '00713':'統一台灣高息','00900':'永豐ESG','00902':'兆豐藍籌30',
}
...
def get_market_status():
    try:
        ...
        if rsi > 80 or ret > 8: return 'OVERBOUGHT', 5, '過熱'
...
class NanaSystemV525:
    ...
    def run(self):
        ...
        if len(etf_df) > 0:
            ...
            for sym, cnt in etf_df['symbol'].value_counts().head(6).items():
        ...
        print(f'  Avg >3.0%:    {"✅" if avg >= 3.0 else "❌"} ({avg:.2f}%)')

        # 報告
        report = {
            'version': 'v5.25_full_backtest',
            ...
            'etf_pool': CORE_ETF,
            'fixes_applied': [
                'BLACKLIST_13_ADDED_4938_2353',
                'REFINE_STOCK_POOL_4_CORES',
                'REMOVE_WEAK_ETF_00662_00646',
                'MAINTAIN_ETF_POOL_6_SELECTED',
                'TIER1_2382_3017_TIER2_2474_2884',
                'REFINE_Avg_TARGET_3.0PCT',
            ]
        }
        with open(f'{TEAM_DIR}/nana_v525_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        print(f'\n報告已儲存: {TEAM_DIR}/nana_v525_report.json')
