# -*- coding: utf-8 -*-
"""
Nana v5.26 - 核心修正：移除2884，加入2345智邦 (Cycle 38)
目標: WR>55%, Avg>2.0%, 交易量>300筆

核心修正 (相較 v5.25):
1. 移除 2884 玉山金：WR=40%, Avg=-0.66%（拖累整體Avg）
2. 加入 2345 智邦：v5.24 WR=61.1%, Avg=2.24%（替補進場）
3. 新股票池：2382, 3017, 2474, 2345（4檔核心）
4. 黑名單維持13檔：2451,2330,1605,6230,2454,2308,3034,3413,2458,2379,8081,4938,2353
5. ETF 池維持6檔：0050, 0056, 00891, 00713, 00900, 00902
"""
import sys
...
# ==================== 精選股票池 (核心4檔) ====================
# v5.26: 移除2884，加入2345智邦
CORE_STOCKS = [
    '2382',  # 廣達  - Tier1, v5.24: 167筆, WR=56.3%, Avg=0.26%
    '3017',  # 奇鋐  - Tier1, v5.24: 157筆, WR=65.0%, Avg=3.51% 🏆
    '2474',  # 可成  - Tier2, v5.24: 119筆, WR=47.9%, Avg=-0.20%
    '2345',  # 智邦  - Tier2, v5.24: WR=61.1%, Avg=2.24%（新加入替補2884）
]
...
class NanaSystemV526:
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.26 - 核心修正：移除2884，加入2345智邦 (Cycle 38)')
        print('='*70)
        print(f'市場狀態: {self.status_desc} | 最大持有: {self.max_hold}天')
        print(f'黑名單: {len(BLACKLIST)} 檔 (維持不變)')
        ...
        report = {
            ...
            'etf_pool': CORE_ETF,
            'fixes_applied': [
                'REMOVE_2884_YushanGold',
                'ADD_2345_ZhiBang_REPLACE_2884',
                'NEW_STOCK_POOL_2382_3017_2474_2345',
                'TIER1_2382_3017_TIER2_2474_2345',
                'TARGET_Avg_2.0PCT',
            ]
        }
        with open(f'{TEAM_DIR}/nana_v526_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        print(f'\n報告已儲存: {TEAM_DIR}/nana_v526_report.json')
