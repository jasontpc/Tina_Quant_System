# -*- coding: utf-8 -*-
"""
Nana v5.27 - 核心修正：提高 ETF 進場頻率，縮短持有期至 4 天 (Cycle 39)
目標: WR>55%, Avg>2.5%, 交易量>200筆

核心修正 (相較 v5.26):
1. ETF 進場頻率大幅提升：降低 score_min 門檻，擴大 Bias 範圍
2. 持有期縮短至 4 天（ETF 與股票一致）
3. 新增 ETF：00646, 00662（完整 8 檔 ETF 池）
4. OVERBOUGHT 市場允許更大 Bias 進場（Bias limit 20→25）
5. ETF OVERBOUGHT score_min: 18→12（大幅降低門檻）
6. ETF OVERBOUGHT bias 範圍：-5~8 → -5~12
"""
import sys
...
# ==================== 精選股票池 (核心4檔) ====================
CORE_STOCKS = [
    '2382',  # 廣達  - Tier1, v5.26: WR=61.1%, Avg=2.24% 🏆
    '3017',  # 奇鋐  - Tier1, WR=87.5%, Avg=5.69% 🏆🏆
    '2474',  # 可成  - Tier2
    '2345',  # 智邦  - Tier2, WR=61.1%, Avg=2.24%
]

# ==================== 精選 ETF 池 (8檔) v5.27====================
# v5.27: 新增 00646, 00662（完整 8 檔）
CORE_ETF = [
    '0050',   # 元大台灣50    - Tier3 ETF
    '0056',   # 元大高股息   - Tier3 ETF
    '00646',  # 國泰台灣5G   - Tier3 ETF (新加入)
    '00662',  # 富邦上証180  - Tier3 ETF (新加入)
...
STOCK_NAMES = {
    ...
    '0050':'元大台灣50','0056':'元大高股息','00646':'國泰台灣5G',
    '00662':'富邦上証180','00713':'統一台灣高息','00891':'中信金特',
    '00900':'永豐ESG','00902':'兆豐藍籌30',
}
...
def get_market_status():
    try:
        ...
        if rsi > 80 or ret > 8: return 'OVERBOUGHT', 4, '過熱'  # v5.27: max_hold 5→4
        elif curr > ma20 and rsi > 55: return 'BULLISH', 4, '多頭'  # v5.27: 5→4
        elif curr < ma20 and rsi < 45: return 'BEARISH', 3, '空頭'
        else: return 'NEUTRAL', 4, '盤整'  # v5.27: 5→4
...
# ==================== ETF 回測 v5.27====================
# 核心修正：降低 score_min，大幅提高進場頻率
def backtest_etf(symbol, max_hold=4, status='OVERBOUGHT'):
    try:
        ...
        # v5.27: 大幅降低 score_min，提高進場頻率
        # OVERBOUGHT: 18→12, BULLISH: 25→18, NEUTRAL: 22→15
        if status == 'OVERBOUGHT':
            score_min = 12  # 18→12（大幅降低）
            rsi_limit = 92; bias_limit = 25; atr_th = 0.10  # bias_limit: 20→25
        elif status == 'BULLISH':
            score_min = 18  # 25→18
            rsi_limit = 70; bias_limit = 14; atr_th = 0.3
        else:
            score_min = 15  # 22→15
            rsi_limit = 75; bias_limit = 14; atr_th = 0.2
        ...
        for i in range(60, len(dates)):
            ...
            if position is None:
                ...
                exit_bias = 25 if status == 'OVERBOUGHT' else 15  # bias_limit 20→25
...
class NanaSystemV527:
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.27 - 提高 ETF 頻率，縮短持有期至 4天 (Cycle 39)')
        print('='*70)
        ...
        print(f'股票分級: Tier1=2382/3017, Tier2=2474/2345')
        print()
        ...
        if len(stock_df) > 0:
            ...
            for sym, cnt in stock_df['symbol'].value_counts().head(10).items():
                ...
                avg_s2 = sdf['profit'].mean()
                t = sdf['tier'].iloc[0]
                print(f'      T{t} {sym} {name(sym)}: {cnt}筆 | WR={wr_s2:.1f}% | Avg={avg_s2:.2f}%')
        ...
        print(f'  Avg >2.5%:    {"✅" if avg >= 2.5 else "❌"} ({avg:.2f}%)')

        # 報告
        report = {
            'version': 'v5.27_full_backtest',
            ...
            'etf_pool': CORE_ETF,
            'fixes_applied': [
                'ETF_SCORE_MIN_LOWERED_OVERBOUGHT_18_TO_12',
                'ETF_SCORE_MIN_LOWERED_BULLISH_25_TO_18',
                'ETF_SCORE_MIN_LOWERED_NEUTRAL_22_TO_15',
                'ETF_BIAS_RANGE_EXPANDED_OVERBOUGHT_8_TO_12',
                'ETF_BIAS_LIMIT_RAISED_20_TO_25',
                'HOLDING_PERIOD_REDUCED_5_TO_4',
                'ADDED_00646_00662_ETF_POOL_8',
                'TARGET_Avg_2.5PCT',
            ]
        }
        with open(f'{TEAM_DIR}/nana_v527_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        print(f'\n報告已儲存: {TEAM_DIR}/nana_v527_report.json')
