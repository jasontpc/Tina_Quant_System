# -*- coding: utf-8 -*-
"""
Nana v5.5 - 交易量擴充修正 (Ultra-Relaxed for OVERBOUGHT)
目標: 極限提高交易數量
"""
import sys
...
def analyze(symbol, status='OVERBOUGHT'):
    try:
        ...
        # ===== ULTRA-RELAXED ENTRY FOR OVERBOUGHT =====
        if status == 'OVERBOUGHT':
            # Tier1: 25分, Tier2: 22分, Tier3: 20分
            tier_score_min = {1: 25, 2: 22, 3: 20}
            score_min = tier_score_min.get(tier, 25)
            # RSI < 75 (放寬), Bias < 15% (大幅放寬), ATR >= 0.15% (大幅放寬)
            can_trade = bool(total >= score_min and rsi < 75 and ma20 > ma60 and atr_pct >= 0.15 and bias < 15 and vol_ratio >= 0.4)
        elif status == 'BULLISH':
            tier_score_min = {1: 30, 2: 27, 3: 25}
            score_min = tier_score_min.get(tier, 30)
            can_trade = bool(total >= score_min and rsi < 68 and ma20 > ma60 and atr_pct >= 0.3 and bias < 10)
        else:
            tier_score_min = {1: 28, 2: 25, 3: 22}
            score_min = tier_score_min.get(tier, 28)
            can_trade = bool(total >= score_min and rsi < 70 and ma20 > ma60 and atr_pct >= 0.2 and bias < 12)
        
        # Inst reversal check (only for non-OVERBOUGHT or skip in extreme relax mode)
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        ...
        inst_reversal = False
        if len(inst_rows) >= 2 and status != 'OVERBOUGHT':
...
class NanaSystemV55:
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.5 - Ultra-Relaxed OVERBOUGHT')
        print('='*70)
        ...
        print()
        print('-'*70)
        print(f' 可交易標的 (v5.5 Ultra-Relaxed)')
        print('-'*70)
        if self.top_picks:
            for r in self.top_picks:
                chg_str = f"{r['today_chg']:+.2f}%"
                tier_icon = {1:'⭐', 2:'~', 3:'-'}
                print(f'  {tier_icon.get(r["tier"],"?")} [{r["score"]:.1f}] {r["symbol"]} {r["name"]} - RSI={r["rsi"]}, F={r["f_days"]}d, bias={r["bias"]:.1f}%, {chg_str}')
        else:
            print('  無符合條件的標的')
        print()
        ...
        report = {
            ...
            'top_picks': [{'symbol': r['symbol'], 'name': r['name'], 'score': r['score'], 'rsi': r['rsi'], 'bias': r['bias']} for r in self.top_picks[:10]],
            'fixes_applied': ['OVERBOUGHT_Score_25_22_20', 'RSI<75', 'Bias<15%', 'ATR>=0.15%', 'vol_ratio>=0.4', 'skip_inst_reversal_in_overbought']
        }
