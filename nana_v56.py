# -*- coding: utf-8 -*-
"""
Nana v5.6 - Tier 分級差異化策略
目標: Tier1/2 緊縮改善品質，Tier3 維持 Ultra-Relaxed
"""
import sys
...
def analyze_v56(symbol, status='OVERBOUGHT'):
    """v5.6 差異化 Tier 分析"""
    try:
        ...
        last_vol_ma5 = last_valid(pd.Series(vol_ma5_arr), vol_arr[-1] if len(vol_arr)>0 and not np.isnan(vol_arr[-1]) else 1.0)
        last_vol = vol_arr[-1] if len(vol_arr)>0 and not np.isnan(vol_arr[-1]) else 1.0
        vol_ratio = round(last_vol / max(last_vol_ma5, 1), 2)
        ...
        # ===== v5.6 TIER DIFFERENTIATED ENTRY =====
        # Tier1: 緊縮 (Score >= 25, RSI < 73, Bias < 13%)
        # Tier2: 中等 (Score >= 22, RSI < 75, Bias < 15%)  
        # Tier3: Ultra (Score >= 20, RSI < 78, Bias < 18%)
        
        if tier == 1:
            score_min = 25; rsi_max = 73; bias_max = 13.0
        elif tier == 2:
            score_min = 22; rsi_max = 75; bias_max = 15.0
        else:  # Tier3 - 維持 Ultra
            score_min = 20; rsi_max = 78; bias_max = 18.0
        
        can_trade = bool(total >= score_min and rsi < rsi_max and ma20 > ma60 and atr_pct >= 0.15 and bias < bias_max and vol_ratio >= 0.4)
        
        # Inst reversal check (skip for OVERBOUGHT Tier3)
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        ...
        inst_reversal = False
        if len(inst_rows) >= 2 and not (status == 'OVERBOUGHT' and tier == 3):
            sell_days = sum(1 for f, in inst_rows if f and f < 0)
        ...
        return {'symbol': symbol, 'name': name(symbol), 'tier': tier,
            ...
            'score_min': score_min, 'rsi_max': rsi_max, 'bias_max': bias_max}
    except Exception as e:
        return None

def backtest_v56(symbol, max_hold=4, status='OVERBOUGHT'):
    """v5.6 差異化 Tier 回測"""
    try:
        ...
        # v5.6 Tier differentiated entry
        if tier == 1:
            score_min = 25; rsi_max = 73; bias_max = 13.0; atr_th = 0.15
        elif tier == 2:
            score_min = 22; rsi_max = 75; bias_max = 15.0; atr_th = 0.15
        else:  # Tier3 - Ultra
            score_min = 20; rsi_max = 78; bias_max = 18.0; atr_th = 0.15
        ...
        for i in range(60, len(dates)):
            ...
            if position is None:
                if total >= score_min and r < rsi_max and m20 > m60 and a >= atr_th and b < bias_max:
                    position = {'entry': price, 'days': 0, 'score': total, 'bias': b}
            else:
                position['days'] += 1
                exit = False; reason = 'time'
                
                # v5.6 Dynamic exit based on tier
                if tier == 1:
                    # Tier1: 緊縮退出 (Bias>12% 提前出)
                    exit_rsi = 80; exit_bias = 12.0
                elif tier == 2:
                    # Tier2: 中等退出 (Bias>14%)
                    exit_rsi = 82; exit_bias = 14.0
                else:
                    # Tier3: Ultra 維持 (RSI>80, Bias>18%)
                    exit_rsi = 85; exit_bias = 18.0
...
class NanaSystemV56:
    ...
    def scan_universe(self):
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Scanning {len(ALL_STOCKS)} stocks (v5.6 Tier-Diff)...')
        for symbol in ALL_STOCKS:
            r = analyze_v56(symbol, self.status)
            if r:
    ...
    def backtest_all(self):
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Backtesting (max_hold={self.max_hold}days, status={self.status})...')
        all_trades = []
        for r in self.top_picks:
            symbol = r['symbol']
            trades = backtest_v56(symbol, self.max_hold, self.status)
            if trades:
                for t in trades:
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.6 - TIER DIFFERENTIATED')
        print('='*70)
        print(f'市場狀態: {self.status_desc} | 最大持有: {self.max_hold}天')
        print()
        print('【v5.6 策略差異】')
        print('  Tier1: Score>=25, RSI<73, Bias<13%  (緊縮)')
        print('  Tier2: Score>=22, RSI<75, Bias<15%  (中等)')
        print('  Tier3: Score>=20, RSI<78, Bias<18%  (Ultra維持)')
        print()
        ...
        print('-'*70)
        for i, r in enumerate(self.results[:25], 1):
            tier_icon = {1:'*', 2:'~', 3:'-'}
            icon = tier_icon.get(r['tier'], '?')
            chg_str = f"{r['today_chg']:+.2f}%"
            trade_tag = '✓' if r['can_trade'] else ' '
            print(f'{i:<4} {r["symbol"]:<8} {r["name"]:<8} {icon}{r["tier"]:<5} {r["score"]:<6.1f} {r["f_days"]:<6} {r["rsi"]:<6.1f} {r["bias"]:<6.1f} {r["atr"]:<6} {chg_str}{trade_tag}')
        
        print()
        print('-'*70)
        print(f' 可交易標的 (v5.6 Tier-Diff)')
        print('-'*70)
        if self.top_picks:
            for r in self.top_picks:
                chg_str = f"{r['today_chg']:+.2f}%"
                tier_icon = {1:'⭐', 2:'~', 3:'-'}
                print(f'  {tier_icon.get(r["tier"],"?")} [{r["score"]:.1f}] {r["symbol"]} {r["name"]} - RSI={r["rsi"]}, F={r["f_days"]}d, bias={r["bias"]:.1f}% ({r["bias_max"]:.1f}%), {chg_str}')
        else:
            print('  無符合條件的標的')
        print()
        ...
        report = {
            'version': 'v5.6_tier_differentiated',
            ...
            'v56_fixes': {
                'tier1_entry': 'Score>=25, RSI<73, Bias<13%',
                'tier2_entry': 'Score>=22, RSI<75, Bias<15%',
                'tier3_entry': 'Score>=20, RSI<78, Bias<18% (Ultra)',
                'tier1_exit': 'RSI>=80 OR Bias>=12%',
                'tier2_exit': 'RSI>=82 OR Bias>=14%',
                'tier3_exit': 'RSI>=85 OR Bias>=18%'
            },
            'top_picks': [{'symbol': r['symbol'], 'name': r['name'], 'score': r['score'], 'rsi': r['rsi'], 'bias': r['bias'], 'tier': r['tier']} for r in self.top_picks[:10]],
        }
        os.makedirs(TEAM_DIR, exist_ok=True)
        with open(f'{TEAM_DIR}/nana_v56_report.json', 'w', encoding='utf-8') as f:
            json.dump(report, f, ensure_ascii=False, indent=2)
        print(f'\n報告已儲存: {TEAM_DIR}/nana_v56_report.json')
