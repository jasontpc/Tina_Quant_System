...
layers = [
    ("Layer1-能源", ["CEG","BE","VST"]),
    ("Layer2-半導體", ["NVDA","INTC","AMD"]),
    ("Layer3-記憶體", ["MU","SNDK","WDC","STX"]),
    ("Layer4-網路", ["AVGO","MRVL","ANET"]),
    ("Layer5-系統散熱", ["VRT","SMCI"]),
    ("Layer6-模型平台", ["MSFT","META","GOOGL"]),
    ("Layer7-代理應用", ["PLTR","CRWD","NOW"]),
]
...
print(f"AI 七層蛋糕技術掃描  2026-05-12")
print(f"{'Layer':<20}{'Ticker':<8}{'Price':>9}{'MA20':>9}{'MA60':>9}{'RSI':>7}{'MACD':>9}{'J':>8}{'1D%':>7}{'5D%':>7}{'20D%':>8} Sharpe  MDD")
print("-"*130)
...
for layer_name, syms in layers:
    ...
    for sym in syms:
        try:
            ...
            sh = stats['sharpe'] or 0; mdd_v = stats['mdd'] or 999
            bull = 0
            if p > float(m20[-1]): bull += 1
            if p > float(m60[-1]): bull += 1
            if float(mh[-1]) > 0: bull += 1
            if j0>j1 and 10<j0<85: bull += 1
            mp = (sh >= 1.5 and mdd_v <= 0.15 and stats['win_rate'] >= 0.45)
            tag = "[BUY]" if (bull>=3 and mp) else "[WATCH]" if (mp) else "[NEUT]"
            print(f"{'':<20}{sym:<8}{p:>9.2f}{float(m20[-1]):>9.2f}{float(m60[-1]):>9.2f}"
                  f"{rs:>7.1f}{float(mh[-1]):>9.3f}{j0:>8.1f}"
                  f"{chg1:>6.1f}%{chg5:>6.1f}%{chg20:>7.1f}%"
                  f"  {sh:.3f}  {mdd_v:.2%} {tag}")
        except Exception as e:
            print(f"{'':<20}{sym:<8} error: {e}")
