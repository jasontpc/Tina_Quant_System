# -*- coding: utf-8 -*-
"""Nana v5.7 - Target Avg >2% via PARTIAL EXIT at +1.5%
============================================================
v5.6 problem: hold_max (36 trades, 47.2% WR, -0.20%) drags avg down
v5.7 fix: partial exit at +1.5% (sell 50%, trail remaining with BE stop)
          + max_hold reduced 3→2 days

Expected impact:
  - Partial exits capture 50% of winning trades early
  - BE stop protects remaining 50% from turning into losses
  - hold_max reduced from 36 → ~20 trades
============================================================
"""
import sys
...
def get_market_status():
    try:
        ...
        if rsi > 80 or ret > 8: return 'OVERBOUGHT', 2, '過熱'
...
def backtest(symbol, max_hold=2, tier=1):
    """v5.7: PARTIAL EXIT strategy
    - Entry: score >= tier threshold, RSI<70, MA20>MA60, ATR>=0.2%, Bias<12%
    - Partial exit: at +1.5%, sell 50% → record profit_1pct_half
    - After partial, set BE stop for remaining 50%
    - Full exit: +2% → sell remaining 50%
    - Stop loss: -1.5x ATR
    - max_hold: 2 days (reduced from 3)
    """
    try:
        ...
        for i in range(60, len(dates)):
            ...
            if position is None:
                ...
                if total >= score_min and r < 70 and m20 > m60 and a >= 0.2 and b < 12:
                    position = {'entry': price, 'days': 0, 'score': total, 'bias': b, 'atr': a,
                                'partial_exited': False, 'partial_profit': 0}
            else:
                position['days'] += 1
                current_profit_pct = (price / position['entry'] - 1) * 100
                exit = False; reason = 'hold_max'
                
                # v5.7: PARTIAL EXIT at +1.5% (sell 50%)
                if not position['partial_exited'] and current_profit_pct >= 1.5:
                    # Record partial exit profit: 50% of position at +1.5%
                    partial = 1.5
                    position['partial_profit'] = partial
                    position['partial_exited'] = True
                    # Remaining 50% now has BE stop
                    position['be_stop'] = position['entry']
                    reason = 'partial_1pct'  # Track but don't exit yet
                    exit = False  # Continue holding remaining 50%
                
                # v5.7: Full exit at +2% (remaining 50%)
                if not exit and position['partial_exited'] and current_profit_pct >= 2.0:
                    exit = True; reason = 'profit_2pct'
                
                # v5.7: BE stop for remaining 50% after partial exit
                elif not exit and position.get('partial_exited') and position.get('be_stop'):
                    if price <= position['be_stop']:
                        exit = True; reason = 'be_stop'
                
                # v5.7: ATR stop loss (only for non-partial trades, or full loss)
                elif not exit and not position['partial_exited'] and current_profit_pct <= -1.5 * position['atr']:
                    exit = True; reason = 'stop_loss'
                
                # Max hold: 2 days
                elif position['days'] >= max_hold:
                    if position['partial_exited']:
                        # Had partial exit, close remaining at market
                        exit = True; reason = 'hold_max_partial'
                    else:
                        exit = True; reason = 'hold_max'
                
                # RSI overbought
                elif not exit and r >= 80:
                    exit = True; reason = 'rsi_overbought'
                
                # Bias too high
                elif not exit and b >= 12:
                    exit = True; reason = 'bias_high'
                
                # MA death cross
                elif not exit and m20 <= m60:
                    exit = True; reason = 'ma_cross'
                
                if exit:
                    # Calculate total profit from both partial and remaining
                    if position['partial_exited']:
                        # Partial already locked 0.75% profit (50% × 1.5%)
                        # Remaining 50% profit:
                        remaining_profit = (price / position['entry'] - 1) * 100
                        # Weight: half of position got 1.5%, half got remaining_profit
                        total_profit = 0.5 * position['partial_profit'] + 0.5 * remaining_profit
                        profit = total_profit
                    else:
                    ...
                    trades.append({'symbol': symbol, 'entry': position['entry'], 'exit': price,
                        ...
                        'partial_profit': position.get('partial_profit', 0)})
                    position = None
        
        # EOD: close any open position
        if position:
            if position['partial_exited']:
                remaining_profit = (close[-1] / position['entry'] - 1) * 100
                profit = 0.5 * position['partial_profit'] + 0.5 * remaining_profit
            else:
...
class NanaSystem:
    ...
    def backtest_all(self):
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Backtesting v5.7 (max_hold={self.max_hold}d, +1.5% PARTIAL EXIT, +2% full exit)...')
        all_trades = []
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.7 - PARTIAL EXIT @ +1.5% → Avg TARGET >2%')
        print('='*70)
        ...
        print()
        print('-'*70)
        print(f' 可交易標的 (Score>=28-35 by Tier, RSI<70, MA20>MA60, ATR>=0.2%, Bias<12%)')
        print('-'*70)
        if self.top_picks:
        ...
        if self.trades:
            ...
            partial_df = df[df.get('partial', False) == True]
            if len(partial_df) > 0:
                print(f'\n【Partial Exit 分析】')
                print(f'  Partial exits: {len(partial_df)}筆 | WR={len(partial_df[partial_df["profit"]>0])/len(partial_df)*100:.1f}% | Avg={partial_df["profit"].mean():.2f}%')
        ...
        report = {
            ...
            'v57_fixes': {
                'partial_exit': '+1.5% → sell 50%, trailing BE for remaining 50%',
                'full_exit': '+2% → sell remaining 50%',
                'be_stop': 'After partial, remaining 50% has BE stop',
                'max_hold': '2 days (from 3)',
                'atr_stop': '1.5x ATR for non-partial trades',
            },
            'top_picks': [{'symbol': r['symbol'], 'name': r['name'], 'score': r['score'], 'rsi': r['rsi'], 'bias': r['bias'], 'tier': r['tier']} for r in self.top_picks[:10]],
        }
        os.makedirs(TEAM_DIR, exist_ok=True)
        report_path = f'{TEAM_DIR}/nana_v57_report.json'
