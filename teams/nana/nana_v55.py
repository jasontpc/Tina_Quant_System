# -*- coding: utf-8 -*-
"""Nana v5.5 - Further relaxed for Tier 2/3 + inst_reversal fix"""
import sys
...
def analyze(symbol):
    try:
        ...
        # v5.5 ENTRY RULES (further relaxed for Tier 2/3):
        # RSI < 70, Bias < 12%, ATR >= 0.2%, vol_ratio >= 0.5
        # Tier-based score thresholds (more relaxed for Tier 2/3)
        ...
        # v5.5: Relax inst_reversal - only block if RSI > 75 (strict)
        conn = sqlite3.connect(DB_PATH)
        cur = conn.cursor()
        ...
        if len(inst_rows) >= 2:
            ...
            if sell_days >= 2 and rsi > 75:  # v5.5: raised from 65 to 75
                inst_reversal = True
                can_trade = False
...
def backtest(symbol, max_hold=5, tier=1):
    try:
        ...
        for i in range(60, len(dates)):
            ...
            if position is None:
                # v5.5 entry: RSI < 70, Bias < 12%, ATR >= 0.2%, tier-based score
                tier_score_min = {1: 35, 2: 30, 3: 28}
                score_min = tier_score_min.get(tier, 35)
...
class NanaSystem:
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.5 - RSI<70 + BIAS<12% + TIER2/3_RELAXED + INST_REVERSAL>75')
        print('='*70)
        ...
        report = {
            ...
            'fixes_applied': ['RSI<70_relaxed', 'Bias<12%', 'ATR>=0.2%', 'vol_ratio>=0.5', 'Tier2/3_relaxed', 'Inst_reversal_RSI>75'],
        }
