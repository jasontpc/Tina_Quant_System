# -*- coding: utf-8 -*-
"""Nana v5.6 - Target Avg >2% by adding profit-taking + tighter stops"""
import sys
...
def backtest(symbol, max_hold=3, tier=1):
    """v5.6 key changes:
    - Exit at +2% profit immediately
    - Stop loss tightened to 1.5x ATR
    - MA golden cross as additional entry confirmation
    """
    try:
        ...
        for i in range(60, len(dates)):
            ...
            if position is None:
                tier_score_min = {1: 35, 2: 30, 3: 28}
                score_min = tier_score_min.get(tier, 35)
                # v5.6: Check MA golden cross (MA20 crossing above MA60 at this bar)
                ma_golden_cross = (i > 0 and ma20[i-1] <= ma60[i-1] and m20 > m60)
                ...
                # v5.6 KEY CHANGE 1: Take profit at +2%
                if current_profit_pct >= 2.0:
                    exit = True; reason = 'profit_2pct'
                # v5.6 KEY CHANGE 2: Tighter stop loss (1.5x ATR instead of 2x)
...
class NanaSystem:
    ...
    def backtest_all(self):
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Backtesting (max_hold={self.max_hold}days, +2% profit target)...')
        all_trades = []
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.6 - TARGET Avg>2% via +2% profit exit + 1.5x ATR stop')
        print('='*70)
        ...
        report = {
            ...
            'fixes_applied': ['+2% profit target exit', '1.5x ATR stop loss', 'max_hold=3', 'RSI<70', 'Bias<12%'],
        }
