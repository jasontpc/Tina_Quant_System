# -*- coding: utf-8 -*-
"""Nana v5.8 - Target Avg >2% via HIGHER SCORE THRESHOLD
============================================================
v5.7 failed (Avg=0.29%) because BE stop on partial exit remaining 50% cancelled gains.
v5.8 fix: instead of partial exit, RAISE entry threshold.
  - Only trade stocks that are MOST likely to reach +2%
  - Tier1 Score>=40 (from 35), Tier3 Score>=32 (from 28)
  - Keep v5.6 logic: +2% full exit, 1.5x ATR stop, max_hold=3

Hypothesis: fewer but HIGHER-QUALITY trades → higher avg return
============================================================
"""
import sys
...
def analyze(symbol):
    try:
        ...
        ma20_s = pd.Series(close).rolling(20).mean(); ma60_s = pd.Series(close).rolling(60).mean()
        ma20 = last_valid(ma20_s, 0); ma60 = last_valid(ma60_s, 0)
        rsi_vals = calc_rsi(close); rsi = rsi_vals[-1]
        atr_pct_arr = calc_atr(close, high, low); atr_pct = last_valid(pd.Series(atr_pct_arr), 0)
        bias_arr = (close - ma20_s.values) / ma20_s.values * 100; bias = last_valid(pd.Series(bias_arr), 0)
        ...
        last_vol_ma5 = vol_ma5_arr[-1] if len(vol_ma5_arr) > 0 and not np.isnan(vol_ma5_arr[-1]) else 1.0
        last_vol = vol_arr[-1] if len(vol_arr) > 0 and not np.isnan(vol_arr[-1]) else 1.0
        vol_ratio = round(last_vol / last_vol_ma5, 2) if last_vol_ma5 > 0 else 1.0
        ...
        # v5.8: RAISE thresholds to trade only high-momentum stocks
        tier_score_min = {1: 40, 2: 35, 3: 32}  # RAISED from {1:35, 2:30, 3:28}
        score_min = tier_score_min.get(tier, 40)
        ...
        if len(inst_rows) >= 2:
            ...
            if sell_days >= 2 and rsi > 75:
                inst_reversal = True; can_trade = False
        return {'symbol': symbol, 'name': name(symbol), 'tier': tier,
            ...
            'f_days': f_days, 't_days': t_days, 'ma20': round(ma20, 0), 'ma60': round(ma60, 0),
            'price': round(close[-1], 0), 'today_chg': round(today_chg, 2),
            'vol_ratio': round(vol_ratio, 2), 'can_trade': can_trade, 'ma20_above': ma20 > ma60,
            'inst_reversal': inst_reversal}
    except Exception as e:
        print(f"  Error analyzing {symbol}: {e}")
        return None

def backtest(symbol, max_hold=3, tier=1):
    """v5.8: Same as v5.6 (proven to work), but with higher entry thresholds.
    Entry: Tier1 Score>=40, Tier2 Score>=35, Tier3 Score>=32
    Exit: +2%, 1.5x ATR stop, max_hold=3, RSI>=80, Bias>=12%, MA cross
    """
    try:
        ...
        ma20 = pd.Series(close).rolling(20).mean().values; ma60 = pd.Series(close).rolling(60).mean().values
        rsi = calc_rsi(close); atr_pct = calc_atr(close, high, low); bias_arr = (close - ma20) / ma20 * 100
        conn = sqlite3.connect(DB_PATH); cur = conn.cursor()
        ...
        tier_score_min = {1: 40, 2: 35, 3: 32}  # v5.8: RAISED thresholds
        score_min = tier_score_min.get(tier, 40)
        for i in range(60, len(dates)):
            price = close[i]; r = rsi[i]
            m20 = ma20[i] if not np.isnan(ma20[i]) else 0; m60 = ma60[i] if not np.isnan(ma60[i]) else 0
            a = atr_pct[i] if not np.isnan(atr_pct[i]) else 0; b = bias_arr[i] if not np.isnan(bias_arr[i]) else 0
            f_d = t_d = 0
            ...
            f_s = inst_score(f_d); t_s = inst_score(t_d); base = max(f_s, t_s)
            ...
            tech = rsi_s + bias_s + atr_s; trend = (15 if m20 > m60 else 0) + (10 if b > 0 else 5)
            ...
            if position is None:
                ...
                elif current_profit_pct <= -1.5 * position['atr']:
...
class NanaSystem:
    ...
    def backtest_all(self):
        print(f'[{datetime.now().strftime("%H:%M:%S")}] Backtesting v5.8 (max_hold={self.max_hold}d, +2% profit, RAISED thresholds)...')
        all_trades = []
    ...
    def run(self):
        print('='*70)
        print(' NANA SYSTEM v5.8 - RAISED ENTRY THRESHOLDS → Avg TARGET >2%')
        print('='*70)
        ...
        print()
        print('-'*70)
        print(f' 可交易標的 (Score>=40-32 by Tier, RSI<70, MA20>MA60, ATR>=0.2%, Bias<12%)')
        print('-'*70)
        if self.top_picks:
            for r in self.top_picks[:10]:
                tier_icon = {1:'⭐', 2:'~', 3:'-'}
                print(f'  {tier_icon.get(r["tier"],"?")} [{r["score"]:.1f}] {r["symbol"]} {r["name"]} - RSI={r["rsi"]}, F={r["f_days"]}d, bias={r["bias"]:.1f}%')
        else:
        ...
        report = {
            ...
            'v58_fixes': {
                'entry_threshold': 'Tier1>=40, Tier2>=35, Tier3>=32 (RAISED from 35/30/28)',
                'exit': '+2% full exit, 1.5x ATR stop, max_hold=3, RSI>=80, Bias>=12%',
            },
            'top_picks': [{'symbol': r['symbol'], 'name': r['name'], 'score': r['score'], 'rsi': r['rsi'], 'bias': r['bias'], 'tier': r['tier']} for r in self.top_picks[:10]],
        }
        os.makedirs(TEAM_DIR, exist_ok=True)
        report_path = f'{TEAM_DIR}/nana_v58_report.json'
