# -*- coding: utf-8 -*-
"""
Vogel v3.0 - Â§öÁ??•Âè∞?áÊ?‰∫§Ê?Á≥ªÁµ±
Á≠ñÁï•1: Bollinger Band Á™ÅÁ†¥ÔºàÂ∑≤Ê∏¨Ë©¶Ôº?Á≠ñÁï•2: RSI ?ûË™ø?•Â†¥
Á≠ñÁï•3: MACD ‰∫§Â?Á¢∫Ë?
Á≠ñÁï•4: Ê∑∑Â?Â§öÂ?Â≠êÁ¢∫Ë™?"""
...
def strategy_bb_break(d, i, data, p):
    """Á≠ñÁï•1: BBÁ™ÅÁ†¥?≤Â†¥ÔºàÊîæÂØ¨Ê?‰ª∂Ô?"""
    rsi = d.get('rsi')
    close = d['close']
    bb_l = d.get('bb_lower')
    bb_u = d.get('bb_upper')
...
def strategy_rsi_reversal(d, i, data, p):
    """Á≠ñÁï•2: RSI ?çË??≤Â†¥ÔºàÊõ¥ÂØ¨È??ÑÈÄ≤Â†¥ÈªûÔ?"""
    rsi = d.get('rsi')
    close = d['close']
    bb_l = d.get('bb_lower')
    ...
    # Long: RSI ÂæûË?Ë≥????(cross above 30) or RSI < 35
    if i > 0:
    ...
    # Long: RSI < 35 ‰∏îÂÉπ?ºÂú® BB ‰∏≠Ë??ÑË??≤Â??ØÊ?
