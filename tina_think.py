...
def expert_risk(data):
    ...
    if twii_rsi > TWII_HOT_RSI:
        score -= 20; signals.append(f'⚠️ 大盤 RSI {twii_rsi:.0f} 過熱')
    elif twii_rsi > 70:
    ...
    if pos_count >= 3:
        score -= 25; signals.append(f'🚫 {data.get("symbol")} 已有{pos_count}口，達上限')
    elif pos_count == 2:
        score -= 10; signals.append(f'⚠️ {data.get("symbol")} 已有{pos_count}口，注意')
    else:
        score += 10; signals.append(f'部位空間充足')
