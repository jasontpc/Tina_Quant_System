...
from us_etf_database import (
    init_db, fetch_analysis, save_analysis,
    generate_dca_signals, get_top_dca, US_ETFS, OPTIMAL_PARAMS
)
import json
...
def main():
    ...
    print(f"\n  {'Symbol':<8} {'Name':<20} {'Price':>10} {'RSI':>6} {'Signal':<12} {'Weight':>6}")
    print("  " + "-" * 70)
    
    for s in dca_priority:
        print(f"  {s['symbol']:<8} {s['name']:<20} ${s['price']:>9.2f} {s['rsi_14']:>6.1f} {s['recommendation']:<12} {s['dca_weight']:>6}%")
    ...
    print(f"\n  {'Sym':<8} {'Name':<20} {'Price':>10} {'Chg':>7} {'RSI':>6} {'Signal':<12} {'Tier':>5}")
    print("  " + "-" * 75)
    ...
    for d in sorted(analysis, key=lambda x: (x.get('tier', 99), -x.get('score', 0))):
        ...
        print(f"  {tier_marker}{d['symbol']:<7} {d['name']:<20} ${d['price']:>9.2f} {sign}{d['change_pct']:>6.1f}% {d['rsi_14']:>6.1f} {d['signal']:<12} {d['tier']:>5}")
    ...
    top_dca = get_top_dca()
    print("\n  Top DCA Picks:")
    for i, d in enumerate(top_dca[:5], 1):
