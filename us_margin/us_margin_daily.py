...
if __name__ == '__main__':
    # Tina 標準化入口
    std = ScriptStandard('us_margin', 'US')
    ...
    try:
        ...
        if report_file.exists():
            with open(report_file, 'r', encoding='utf-8') as f:
                report = json.load(f)
            signals = report.get('high_risk', []) + report.get('short_squeeze', [])
            metrics = {
                'high_risk_count': report.get('high_risk_count', 0),
                'alerts_count': report.get('alerts_count', 0),
                'squeeze_count': len(report.get('short_squeeze', []))
            }
        else:
            signals = []
            metrics = {}
        ...
        std.handle_error(e, 'us_margin_daily.py 執行失敗')
